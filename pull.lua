...
function downloadManager(path)
    ...
    for i, data in pairs(fType) do
        if data == "file" then
            ...
            if gPath ~= "" then
                path = gPath .. "/" .. gRepo .. "/" .. fPath[i]
            end
            if not fileList.files[path] and not isBlackListed(fPath[i]) then
                fileList.files[path] = {"https://raw.github.com/" .. gUser .. "/" .. gRepo .. "/" .. gBranch .. "/" ..
                    fPath[i], fName[i]}
            end
        end
    end
    for i, data in pairs(fType) do
        if data == "dir" then
            ...
            if not fileList.dirs[path] then
                ...
                fileList.dirs[path] = {"https://raw.github.com/" .. gUser .. "/" .. gRepo .. "/" .. gBranch .. "/" ..
                    fPath[i], fName[i]}
...
if not http then
    ...
    function watchOsEvents()
        listenForEvent({
            http_failure = function(e, url, handle)
                print("error", e, url, handle)
            end,
            http_success = function(s, url, handle)
                print("success", s, url, handle)
            end
        })
    end

    shell.run("delete /MinecraftScripts")
    parallel.waitForAny(watchOsEvents, function()
